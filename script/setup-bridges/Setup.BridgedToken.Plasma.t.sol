...
contract BridgedTokenSetupPlasmaScript is Test {
    ...
    function _createConfigPlasma(
        ...
        require(
            uint(configDeployed.get(PLASMA_CHAIN_ID, "OAPP_MAIN_TOKEN").ty.kind) != 0,
            "Bridged token is not deployed on Plasma"
        );
        address oapp = configDeployed.get(PLASMA_CHAIN_ID, "OAPP_MAIN_TOKEN").toAddress();
