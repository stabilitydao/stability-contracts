...
contract RevenueRouterTestSonic is Test {
    ...
    function test_RevenueRouter_xToken_feeTreasury() public {
        _deployWithXTokenLandFeeTreasury();
        ...
        IERC20(SonicConstantsLib.TOKEN_STBL).approve(address(xToken), 1e18);
        xToken.enter(1e18);
        IERC20(address(xToken)).approve(address(xStaking), 1e18);
    ...
    function testAddresses() public {
        ...
        assertEq(addresses[1], address(xToken));
        assertEq(addresses[2], address(xStaking));
        assertEq(addresses[3], address(feeTreasury));
    }

    function _deployWithXTokenLandFeeTreasury() internal {
