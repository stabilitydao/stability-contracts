...
import {LfjV2Adapter} from "../../src/adapters/LfjV2Adapter.sol";
...
contract LfjV2AdapterTest is AvalancheSetup {
    ...
    constructor() {
        ...
        _hash = keccak256(bytes(AmmAdapterIdLib.LFJGG));
...
// todo        
//        vm.expectRevert();
//        adapter.getPrice(pool, AvalancheConstantsLib.TOKEN_savUSD, AvalancheConstantsLib.TOKEN_AUSDC, 0);
//
//        vm.expectRevert();
//        adapter.getPrice(pool, AvalancheConstantsLib.TOKEN_AUSDC, AvalancheConstantsLib.TOKEN_savUSD, 0);
    }
    ...
    function testStakeBrunchBadPaths() public {
        ...
        vm.expectRevert(LfjV2Adapter.IncorrectTokens.selector);
        adapter.swap(
    ...
    function _testStakeBrunchUSD(uint amount) internal {
        ...
        assertApproxEqAbs(state2.userBalanceAvUSD, state0.userBalanceAvUSD, 1, "all avUSD back");
    }
    ...
    function _addAdapter() internal {
        ...
        proxy.initProxy(address(new LfjV2Adapter()));
        LfjV2Adapter(address(proxy)).init(PLATFORM);
        ...
        adapter = LfjV2Adapter(address(proxy));
    }
