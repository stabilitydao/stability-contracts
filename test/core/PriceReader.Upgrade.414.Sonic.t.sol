...
contract PriceReaderUpgrade414SonicTest is Test {
    ...
    function testGetTwaPriceSolidlyAdapter() public view {
        ...
        (uint twaPrice,) = priceReader.getTwaPrice(SonicConstantsLib.TOKEN_STBL, SonicConstantsLib.POOL_SHADOW_STBL_USDC, AmmAdapterIdLib.SOLIDLY, 300);
...
//        console.log("twaPrice", twaPrice);
    ...
    function testGetTwaPriceSolidlyAdapterZeroPeriodCurrentPrice() public view {
        (uint price,) = priceReader.getPrice(SonicConstantsLib.TOKEN_STBL);
        (uint twaPrice,) = priceReader.getTwaPrice(SonicConstantsLib.TOKEN_STBL, SonicConstantsLib.POOL_SHADOW_STBL_USDC, AmmAdapterIdLib.SOLIDLY, 0);

//        console.log("price", price);
//        console.log("twaPrice", twaPrice);

        assertEq(price, twaPrice, "current price == twa price (assume here that swapper takes price through SolidlyAdapter)");
    }

    function testGetTwaPriceUniswapAdapter() public view {
        (uint price,) = priceReader.getPrice(SonicConstantsLib.TOKEN_STBL);
        (uint twaPrice,) = priceReader.getTwaPrice(SonicConstantsLib.TOKEN_STBL, SonicConstantsLib.POOL_SHADOW_CL_STBL_USDC, AmmAdapterIdLib.UNISWAPV3, 300);
...
//        console.log("twaPrice", twaPrice);
    ...
    function testGetTwaPriceBadPaths() public {
        ...
        priceReader.getTwaPrice(SonicConstantsLib.TOKEN_STBL, SonicConstantsLib.POOL_SHADOW_CL_STBL_USDC, "Unknown", 300);

        // ---------------------------- number of tokens in the pool is not 2
        vm.expectRevert(PriceReader.WrongNumberPoolTokens.selector);
        priceReader.getTwaPrice(SonicConstantsLib.TOKEN_STBL, SonicConstantsLib.POOL_PENDLE_PT_SMSUSD_30OCT2025, AmmAdapterIdLib.PENDLE, 300);
        ...
        (uint price, bool trusted) = priceReader.getTwaPrice(SonicConstantsLib.TOKEN_USDC, POOL_SHADOW_USDC_USSSDC, AmmAdapterIdLib.SOLIDLY, 300);
