...
library StrategyLogicLib {
    ...
    function tokenURI(
        ...
        //region ----- Strategy -----
        vars.h = 328;
        output = string.concat(output, '<rect fill="#', vars.strategyBgColor, '" width="600" height="', _str(vars.strategyBlockHeight), '"/>');
        ...
        output = string.concat(output, '<text transform="matrix(1 0 0 1 ', _str(offset),' ', _str(vars.h), ')" fill="#', vars.strategyColor, '" class="base shortId">', CommonLib.shortId(strategyData.strategyId), '</text>');

        vars.h += 328;
        output = string.concat(output, '<text transform="matrix(1 0 0 1 50 ', _str(vars.h), ')" fill="#', vars.strategyColor, '" class="strategyTitle base">Strategy #', _str(strategyData.strategyTokenId), '</text>');
        vars.h += vars.step + 4;
        output = string.concat(output, '<text transform="matrix(1 0 0 1 50 ', _str(vars.h), ')" fill="#', vars.strategyColor, '" class="strategy base">', strategyData.strategyId, '</text>');
        //endregion -- Strategy -----

        //region ----- Platform -----
        vars.step = 30;
        output = string.concat(output, '<rect y="', _str(vars.strategyBlockHeight), '" fill="#', vars.networkBgColor, '" width="600" height="', _str(900 - vars.strategyBlockHeight), '"/>');
        vars.h = vars.strategyBlockHeight + 20;
        vars.h += vars.step;
        output = string.concat(output, '<text transform="matrix(1 0 0 1 50 ', _str(vars.h), ')" fill="#', vars.networkColor, '" class="platform base">Stability Platform ', platformVersion, '</text>');
        vars.h += vars.step;
        ...
        vars.h += vars.step;
        output = string.concat(output, '<text transform="matrix(1 0 0 1 50 ', _str(vars.h), ')" fill="#', vars.networkColor, '" class="platform-param base">Strategy share</text><text transform="matrix(1 0 0 1 300 ', _str(vars.h), ')" fill="#', vars.networkColor, '" class="platform-value base">', CommonLib.formatApr(platformData.feeShareStrategyLogic), '</text>');
        //endregion -- Platform -----
