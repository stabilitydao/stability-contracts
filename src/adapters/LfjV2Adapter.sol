...
import {ILbPair} from "../integrations/lfjggv2/ILbPair.sol";
import {IERC20Metadata} from "../../lib/openzeppelin-contracts/contracts/token/ERC20/extensions/IERC20Metadata.sol";

/// @title AMM adapter for Liquidity book protocol, see https://lfj.gg/ , https://github.com/lfj-gg
/// @author omriss (https://github.com/omriss)
contract LfjV2Adapter is Controllable, IAmmAdapter {
    ...
    function swap(
        ...
        address tokenX = pair.getTokenX();
        {
            ...
            require(
                (tokenX == tokenIn && tokenY == tokenOut) || (tokenY == tokenIn && tokenX == tokenOut),
                IncorrectTokens()
            );
        }
        ...
        console.log("amount, amountInLeft, price", amount, amountInLeft, price);
        require(amountInLeft == 0, InsufficientOutputAmount());
    ...
    function ammAdapterId() external pure returns (string memory) {
        return AmmAdapterIdLib.LFJGG;
    }

    /// @inheritdoc IAmmAdapter
    function poolTokens(address pool) public view returns (address[] memory tokens) {
        tokens = new address[](2);
        tokens[0] = ILbPair(pool).getTokenX();
        tokens[1] = ILbPair(pool).getTokenY();
    }
    ...
    function getPrice(address pool, address tokenIn, address /*tokenOut*/, uint amount) public view returns (uint) {
        ...
        (uint128 amountInLeft, uint128 amountOut, ) = ILbPair(pool).getSwapOut(uint128(amount), tokenIn == ILbPair(pool).getTokenX());
        console.log("tokenIn, tokenX", tokenIn, ILbPair(pool).getTokenX());
        console.log("getPrice amount, amountOut, amountInLeft", amount, amountOut, amountInLeft);
