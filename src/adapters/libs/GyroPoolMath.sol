...
library GyroPoolMath {
    ...
    uint private constant SQRT_1E_NEG_1 = 316227766016837933;
    uint private constant SQRT_1E_NEG_3 = 31622776601683793;
    uint private constant SQRT_1E_NEG_5 = 3162277660168379;
    uint private constant SQRT_1E_NEG_7 = 316227766016837;
    uint private constant SQRT_1E_NEG_9 = 31622776601683;
    uint private constant SQRT_1E_NEG_11 = 3162277660168;
    uint private constant SQRT_1E_NEG_13 = 316227766016;
    uint private constant SQRT_1E_NEG_15 = 31622776601;
    uint private constant SQRT_1E_NEG_17 = 3162277660;
    ...
    function _calcAllTokensInGivenExactBptOut(
        ...
        uint bptOut,
        uint totalBPT
    ) internal pure returns (uint[] memory amountsIn) {
        /**
         *
    ...
    function _calcProtocolFees(
        ...
        uint currentBptSupply,
        uint protocolSwapFeePerc,
        uint protocolFeeGyroPortion
        ...
        uint numerator =
            (currentBptSupply.mulDown(currentInvariant.sub(previousInvariant))).mulDown(protocolSwapFeePerc);
        uint diffInvariant = protocolSwapFeePerc.mulDown(currentInvariant.sub(previousInvariant));
        uint denominator = currentInvariant.sub(diffInvariant);
        uint deltaS = numerator.divDown(denominator);

        // Split fees between Gyro and Balancer
        uint gyroFees = protocolFeeGyroPortion.mulDown(deltaS);
        uint balancerFees = deltaS.sub(gyroFees);
     ...
     */
    function _sqrt(uint input, uint tolerance) internal pure returns (uint) {
        if (input == 0) {
            return 0;
        }

        uint guess = _makeInitialGuess(input);
        ...
        uint guessSquared = guess.mulDown(guess);
        require(
            guessSquared <= input.add(guess.mulUp(tolerance)) && guessSquared >= input.sub(guess.mulUp(tolerance)),
            "_sqrt FAILED"
        );
    ...
    function _makeInitialGuess(uint input) internal pure returns (uint) {
    ...
    function _intLog2Halved(uint x) public pure returns (uint n) {
        if (x >= 1 << 128) {
            x >>= 128;
            n += 64;
        }
    ...
    function liquidityInvariantUpdate(
        uint uinvariant,
        uint changeBptSupply,
        uint currentBptSupply,
        bool isIncreaseLiq
    ) internal pure returns (uint unewInvariant) {
        //  change in invariant
        if (isIncreaseLiq) {
            // round new invariant up so that protocol fees not triggered
            uint dL = uinvariant.mulUp(changeBptSupply).divUp(currentBptSupply);
            unewInvariant = uinvariant.add(dL);
        } else {
            // round new invariant up (and so round dL down) so that protocol fees not triggered
            uint dL = uinvariant.mulDown(changeBptSupply).divDown(currentBptSupply);
    ...
    function liquidityInvariantUpdate(
        uint[] memory balances,
        uint uinvariant,
        uint[] memory deltaBalances,
        bool isIncreaseLiq
    ) internal pure returns (uint unewInvariant) {
        uint largestBalanceIndex;
        uint largestBalance;
        for (uint i = 0; i < balances.length; i++) {
        ...
        uint deltaInvariant =
            uinvariant.mulDown(deltaBalances[largestBalanceIndex]).divDown(balances[largestBalanceIndex]);
