...
library StableMath {
    ...
    uint public constant MAX_STABLE_TOKENS = 5;

    uint internal constant MIN_AMP = 1;
    uint internal constant MAX_AMP = 5000;
    uint internal constant AMP_PRECISION = 1e3;

    // Invariant growth limit: non-proportional add cannot cause the invariant to increase by more than this ratio.
    uint internal constant MIN_INVARIANT_RATIO = 60e16; // 60%
    // Invariant shrink limit: non-proportional remove cannot cause the invariant to decrease by less than this ratio.
    uint internal constant MAX_INVARIANT_RATIO = 500e16; // 500%
    ...
    function computeOutGivenExactIn(
        ...
        uint finalBalanceOut = computeBalance(amplificationParameter, balances, invariant, tokenIndexOut);
    ...
    function computeBalance(
        ...
        uint inv2 = invariant * invariant;
        // We remove the balance from c by multiplying it.
        uint c = (inv2 * AMP_PRECISION).divUpRaw(ampTimesTotal * P_D) * balances[tokenIndex];
        uint b = sum + ((invariant * AMP_PRECISION) / ampTimesTotal);
        // We iterate to find the balance.
        uint prevTokenBalance = 0;
        // We multiply the first iteration outside the loop with the invariant to set the value of the
        // initial approximation.
        uint tokenBalance = (inv2 + c).divUpRaw(invariant + b);
