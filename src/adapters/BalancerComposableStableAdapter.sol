...
import "./libs/balancer/BalancerErrors.sol";
...
contract BalancerComposableStableAdapter is Controllable, IAmmAdapter, IBalancerAdapter {
    ...
    function poolTokens(address pool) public view returns (address[] memory tokens) {
        ...
        (IERC20[] memory bTokens,,) = IBVault(_pool.getVault()).getPoolTokens(_pool.getPoolId());
        uint bptIndex = _pool.getBptIndex();
        uint len = bTokens.length - 1;
        tokens = new address[](len);
        for (uint i; i < len; ++i) {
            tokens[i] = address(bTokens[i < bptIndex ? i : i + 1]);
        }
    }
    ...
    function getLiquidityForAmountsWrite(
        ...
        (IERC20[] memory tokens,,) = IBVault(_pool.getVault()).getPoolTokens(v.poolId);
        v.len = tokens.length;
        v.assets = new address[](v.len);
        v.bptIndex = _pool.getBptIndex();
        uint k;
        for (uint i; i < v.len; ++i) {
            v.assets[i] = address(tokens[i]);
        }
    ...
    function getProportions(address pool) external view returns (uint[] memory props) {
        ...
        (IERC20[] memory tokens, uint[] memory balances,) = IBVault(_pool.getVault()).getPoolTokens(_pool.getPoolId());
        uint totalInAsset0;
        ...
        for (uint i; i < len; ++i) {
            if (i != v.bptIndex) {
                uint tokenDecimals = IERC20Metadata(address(tokens[i])).decimals();
    ...
    function getPrice(address pool, address tokenIn, address tokenOut, uint amount) public view returns (uint) {
        ...
        (IERC20[] memory tokens, uint[] memory balances,) = IBVault(_pool.getVault()).getPoolTokens(poolId);
