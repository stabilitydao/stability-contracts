...
import {
    LiquidityManagement, PoolConfig, HooksConfig, PoolRoleAccounts,
    TokenConfig, TokenInfo, PoolData, PoolSwapParams
} from "./VaultTypes.sol";
...
interface IVaultExtension {
     ...
     */
    function getNonzeroDeltaCount() external view returns (uint nonzeroDeltaCount);
     ...
     */
    function getTokenDelta(IERC20 token) external view returns (int tokenDelta);
     ...
     */
    function getReservesOf(IERC20 token) external view returns (uint reserveAmount);
    ...
    function getPoolTokenRates(address pool)
        external
        view
        returns (uint[] memory decimalScalingFactors, uint[] memory tokenRates);
    ...
    function getPoolTokenInfo(address pool)
        ...
        returns (
            ...
            uint[] memory balancesRaw,
            uint[] memory lastBalancesLiveScaled18
        );
     ...
     */
    function getCurrentLiveBalances(address pool) external view returns (uint[] memory balancesLiveScaled18);
     ...
     */
    function getBptRate(address pool) external view returns (uint rate);
     ...
     */
    function totalSupply(address token) external view returns (uint tokenTotalSupply);
     ...
     */
    function balanceOf(address token, address account) external view returns (uint tokenBalance);
     ...
     */
    function allowance(address token, address owner, address spender) external view returns (uint tokenAllowance);
     ...
     */
    function approve(address owner, address spender, uint amount) external returns (bool success);
     ...
     */
    function getAggregateSwapFeeAmount(address pool, IERC20 token) external view returns (uint swapFeeAmount);
     ...
     */
    function getAggregateYieldFeeAmount(address pool, IERC20 token) external view returns (uint yieldFeeAmount);
     ...
     */
    function getStaticSwapFeePercentage(address pool) external view returns (uint swapFeePercentage);
    ...
    ) external view returns (uint dynamicSwapFeePercentage);
