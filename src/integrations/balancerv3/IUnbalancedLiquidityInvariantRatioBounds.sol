...
interface IUnbalancedLiquidityInvariantRatioBounds {
    ...
    function getMinimumInvariantRatio() external view returns (uint minimumInvariantRatio);

    /// @return maximumInvariantRatio The maximum invariant ratio for a pool during unbalanced add liquidity
    function getMaximumInvariantRatio() external view returns (uint maximumInvariantRatio);
}
