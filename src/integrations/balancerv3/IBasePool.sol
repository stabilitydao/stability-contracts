...
interface IBasePool is ISwapFeePercentageBounds, IUnbalancedLiquidityInvariantRatioBounds {
    ...
    function computeInvariant(
        uint[] memory balancesLiveScaled18,
        Rounding rounding
    ) external view returns (uint invariant);
    ...
    function computeBalance(
        ...
        uint tokenInIndex,
        uint invariantRatio
    ) external view returns (uint newBalance);
     ...
     */
    function onSwap(PoolSwapParams calldata params) external returns (uint amountCalculatedScaled18);
}
