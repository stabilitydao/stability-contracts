...
import {TokenInfo} from "./VaultTypes.sol";
 ...
 * Simplified for Stability Platform
 */
interface IPoolInfo {
    /**
     ...
     */
    function getCurrentLiveBalances() external view returns (uint[] memory balancesLiveScaled18);
     ...
     */
    function getStaticSwapFeePercentage() external view returns (uint staticSwapFeePercentage);
    ...
    function getAggregateFeePercentages()
        external
        view
        returns (uint aggregateSwapFeePercentage, uint aggregateYieldFeePercentage);
}
