...
interface ISwapFeePercentageBounds {
    ...
    function getMinimumSwapFeePercentage() external view returns (uint minimumSwapFeePercentage);

    /// @return maximumSwapFeePercentage The maximum swap fee percentage for a pool
    function getMaximumSwapFeePercentage() external view returns (uint maximumSwapFeePercentage);
}
