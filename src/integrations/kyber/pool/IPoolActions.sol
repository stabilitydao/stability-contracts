...
interface IPoolActions {
    ...
    function unlockPool(uint160 initialSqrtP) external returns (uint qty0, uint qty1);
    ...
    function mint(
        ...
        uint128 qty,
        bytes calldata data
    ) external returns (uint qty0, uint qty1, uint feeGrowthInside);
    ...
    function burnRTokens(uint qty, bool isLogicalBurn) external returns (uint qty0, uint qty1);
    ...
    function swap(
        ...
        uint160 limitSqrtP,
        bytes calldata data
    ) external returns (int qty0, int qty1);
    ...
    /// @param data Any data to be passed through to the callback
    function flash(address recipient, uint qty0, uint qty1, bytes calldata data) external;

    /// @notice sync fee of position
    /// @param tickLower Position's lower tick
    /// @param tickUpper Position's upper tick
    function tweakPosZeroLiq(int24 tickLower, int24 tickUpper) external returns (uint feeGrowthInsideLast);
}
