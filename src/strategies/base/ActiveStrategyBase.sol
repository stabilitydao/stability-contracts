...
abstract contract ActiveStrategyBase is StrategyBase, IActiveStrategy {
    ...
    /// @inheritdoc IActiveStrategy
    function needRebalanceWithSwap() external view virtual returns (
        bool,
        address[] memory,
        address[] memory,
        uint[] memory
    ...
    function _beforeDeposit() internal override virtual {
        if (_needRebalance()) {
    ...
    /// @inheritdoc StrategyBase
    function _beforeWithdraw() internal override virtual {
        if (_needRebalance()) {
            revert NeedRebalance();
        }
    }
    ...
    /// @inheritdoc StrategyBase
    function _beforeDoHardWork() internal override virtual {
        if (_needRebalance()) {
            revert NeedRebalance();
        }
    }
